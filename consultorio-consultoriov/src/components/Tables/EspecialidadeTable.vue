<template>
  <table class="table" >
    ...
    <tbody>
        <tr v-for="item in especialidadeList" :key="item.id" class="has-text-left">
             <th> <button @click="onClickPaginaDetalhar(item.id)" class="button is-small is-link is-rounded"> Info </button> </th>
            <th>{{item.nome}}</th>
            <th span v-if="item.ativo" class="tag is-link is-rounded">Ativo</th>
            <th span v-if="!item.ativo" class="tag is-danger is-rounded">Inativo</th>
        </tr>
    </tbody>
  </table>  
</template>
...
export default class EspecialidadeTable extends Vue {
    ...
    private onClickPaginaDetalhar(idEspecialidade: number){
      this.$router.push({ name: 'especialidade-detalhar', params: { id: idEspecialidade, model: 'foda' } })
    }
}
</script>
